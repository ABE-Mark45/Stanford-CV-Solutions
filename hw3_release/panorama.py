...
def harris_corners(img, window_size=3, k=0.04):
    ...
    ### YOUR CODE HERE
    ix = convolve(dx**2, window)
    iy = convolve(dy**2, window)
    ixiy = convolve(dx*dy, window)
    f = np.dstack((ix, ixiy))
    g = np.dstack((ixiy, iy))
    a = np.dstack((f,g))
    M = a.reshape((a.shape[0],a.shape[1],2,2))
...
def linear_blend(img1_warped, img2_warped):
    ...
    ### YOUR CODE HERE
    weights_1 = np.hstack((np.ones((out_H, left_margin-1)), np.tile(np.linspace(1, 0, right_margin-left_margin+1), (out_H, 1)), np.zeros((out_H, out_W-right_margin))))
    weights_2 = np.hstack((np.zeros((out_H, left_margin-1)), np.tile(np.linspace(0, 1, right_margin-left_margin+1), (out_H, 1)), np.ones((out_H, out_W-right_margin))))

    merged = img1_warped*weights_1 + img2_warped*weights_2
...
def stitch_multiple_images(imgs, desc_func=simple_descriptor, patch_size=5):
    ...
    for i in range(1, len(imgs)):
        ...
        img2_warped = warp_image(imgs[i], transforms[i-1], output_shape, offset)
